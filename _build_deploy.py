...
checks={
 ...
 'venda herda foto do item':('orig&&orig.fotoThumb' in chk or 'orig&&thumbReal(orig.fotoThumb)' in chk) and 'fotos do item (' in chk,
 ...
 'registro de exclusao viaja pela nuvem (payload + merge + volta)':'codigosResolvidos,excluidos};' in chk and ('excluidos:mergeDictRaso(excluidos,remoto.excluidos),' in chk or 'mergeDictRaso(excluidos,r.excluidos)' in chk or 'mergeExcl(excluidos,r.excluidos)' in chk) and "if(d.excluidos&&typeof d.excluidos==='object')" in chk and "gravaLocal('tcg_excluidos'" in chk,
 ...
 # [fotos F2 23/08] os dois passaram a registrar TAMBEM o tumulo do grupo (nota/venda/troca) — aceita as duas formas
 'apagar tudo e apagar nota registram a exclusao (senao voltavam pela nuvem)':('marcaExcluido(movs.map(x=>x.id));movs=[];' in chk or 'marcaExcluido(movs.map(x=>x.id).concat(gids));movs=[];' in chk) and ('marcaExcluido(its.map(x=>x.id));movs=movs.filter(m=>m.notaId!==nid);' in chk or "marcaExcluido(its.map(x=>x.id).concat(['nota:'+nid]));movs=movs.filter(m=>m.notaId!==nid);" in chk),
 ...
 'picker Janet persiste nuvem+backup+import (replace, nao merge)':'cadastros,contasBanc,codigosResolvidos,excluidos};' in chk and 'const payload={...final,movs:semThumbs(final.movs),_upd:Date.now()' in chk and "codigosResolvidos=d.codigosResolvidos;if(d.excluidos" in chk and 'gravaLocal(MK,JSON.stringify(movs));' in chk and "Object.keys(d.codigosResolvidos).length)codigosResolvidos=d.codigosResolvidos;salvarCad" in chk,
 ...
 'sem internet: lancamento local sobrevive ao snapshot do outro aparelho (pendencia + fusao, F0 22/08)':'function marcaPendNuvem' in chk and 'function fundirComRemoto' in chk and 'const fundiuPend=pendNuvem();' in chk and 'if(fundiuPend){' in chk and ("localStorage.getItem('tcg_pend_nuvem')==='1')_pendSeq=1" in chk or "localStorage.getItem('tcg_pend_nuvem')==='1'){_pendSeq=1;_pendDesde=Date.now();}" in chk) and 'limpaPendNuvem(seqCap)' in chk and chk.count('marcaPendNuvem();if(_syncReady)salvarNuvem();')>=3,
 ...
 # [fotos F2 23/08 — D1 foto de cima = do GRUPO (nota/venda/troca) + visor de grupo + tumulo de grupo + avisos; D2 foto por carta na venda; nucleo do mover foto]
 'fotos F2: comprovante vai para o grupo (nid/vid/tid), foto por carta na venda, visor de grupo, tumulo de grupo, mover foto':'aplicarFotosPend(nid);' in chk and 'aplicarFotosPend(vid);' in chk and 'aplicarFotosPend(tid);' in chk and 'function abrirFotosGrupo' in chk and 'function moverFotosDe' in chk and 'function fotoMove' in chk and 'aplicarFotosDoItem(it.fotos,base.id)' in chk and chk.count("fotos:_fotosItem.slice()});_fotosItem=[];render();")>=2 and "marcaExcluido('nota:'+nid)" in chk and 'function desfazerNotaFaz' in chk and "if(perfil==='doc')" in chk and 'function grupoPend' in chk and "abrirFotosGrupo('${nid}'" in chk and "abrirFotosGrupo('${vid}'" in chk and 'fotos da nota (comprovante)' in chk,
 # [revisao F2 23/08 — fechamentos da revisao adversarial: grupo vazio ganha tumulo em qualquer saida; tirar o ultimo produto pergunta; mover so conclui com n===total e o incompleto vai para a fila; fechar o comprovante reabre o dono; reenvio so redesenha a grade do dono; remover item com foto pergunta; exclusao simples diz onde ficam as fotos]
 'fotos F2b: grupo vazio ganha tumulo (execExcl, separar, soltarIntrusa) e separar tem gesto em 2 passos':'function enterraGruposVazios(removidos)' in chk and chk.count('enterraGruposVazios(')>=3 and 'function separarDaNotaFaz(id)' in chk,
 'fotos F2b: tirar o ultimo produto de nota com comprovante pergunta (3 saidas)':'Tirar o último produto da nota?' in chk and 'Mover as fotos para este produto e tirar da nota' in chk and 'O produto NÃO foi tirado da nota' in chk,
 'fotos F2b: mover fotos so conclui com n===total; gesto incompleto entra na fila e e retomado no commit da nuvem':'cb&&cb(n,total)' in chk and '_movesPendentes.push(' in chk and 'function retomarMovesPendentes()' in chk and 'retomarMovesPendentes();' in chk and 'q===null||q<tot' in chk,
 'fotos F2b: fechar o visor do comprovante reabre o dono (nota/venda), com aspas escapadas':'function abrirFotosGrupo(gid,titulo,voltaJs)' in chk and "abrirNota(&quot;${nid}&quot;)" in chk and "verVenda(&quot;${vid}&quot;)" in chk and "replace(/\"/g,'&quot;')" in chk,
 'fotos F2b: reenvio so redesenha a grade DO dono':'_gradeDe===f.movId' in chk and '_gradeDe=gid;' in chk and '_gradeDe=movId;' in chk,
 'fotos F2b: remover item/recebido com foto da carta pergunta (nota, venda, troca)':chk.count('Remover o item da')>=3,
 'fotos F2b: balde do comprovante descartado ao trocar de modo dentro do Lancar':'function guardaBaldePend()' in chk and 'guardaBaldePend();' in chk and 'do comprovante descartada' in chk,
 'fotos F2b: exclusao simples diz onde ficam as fotos; cartao diz que e o comprovante':('As fotos dele (se houver) ficam guardadas na nuvem' in chk or 'vai para a 🗑 Lixeira (em Backup) por 120 dias' in chk) and "📷 fotos da ${m.notaId?'nota':m.vendaId?'venda':'troca'} (comprovante)" in chk,
 # [re-checagem F2 23/08 — medio 1 devolucao enterra grupo; menor 1 mover nao duplica; medio 3 fila de retomada no disco + retomada na abertura; menor 2 nota inteira enterra troca vazia; menor 3 balde do comprovante muda com o TIPO]
 'fotos F2c: devolucao (execDev) e excluir nota inteira tambem enterram grupo vazio':chk.count('enterraGruposVazios([v]);')>=2 and 'enterraGruposVazios(its);' in chk and chk.count('enterraGruposVazios(')>=6,
 'fotos F2c: repetir o mover nao duplica (le o destino antes) e a fila de retomada vai para o disco e roda na abertura':'const jaLa=new Set((D||[]).map(x=>x.b64));' in chk and 'if(jaLa.has(f.b64)){fotoDel(f.id' in chk and "gravaLocal('tcg_moves_pend'" in chk and "localStorage.getItem('tcg_moves_pend')" in chk and ('setTimeout(()=>{try{retomarMovesPendentes();}catch(e){}},4000);' in chk or 'setTimeout(()=>{try{retomarMovesPendentes();}catch(e){}try{carregarFilaFotos();}catch(e){}try{processarPurgaFotos();}catch(e){}},4000);' in chk) and 'gravaMovesPend();toast(' in chk,
 'fotos F2c: balde do comprovante muda de significado com o TIPO (compra->venda), nao so com o modo':"const gp=(tela==='lancar'?(tipoSel+':'+(grupoPend()||'item')):'fora')" in chk,
 # [fotos F3 23/08 — gesto "mover foto" na UI: candidatos ordenados (irmaos -> comprovante -> 90 dias), fotoMove (grava->apaga), recusa offline com nuvem, reabre a grade de origem, na lista de toques protegidos]
 'fotos F3: botao mover no visor, painel com busca, mecanica fotoMove, recusa sem internet, reabre a grade, toque protegido':"abrirMoverFoto('${fid}','${movId}')" in chk and 'function candidatosMoverFoto(movId,tudo)' in chk and "selBusca('mvf_dest'" in chk and 'function moverFotoPara(fid,movId)' in chk and 'navigator.onLine===false' in chk and 'fotoMove(f,destino,ok=>fim(ok,false,D))' in chk and 'function reabrirGrade(id)' in chk and '|moverFotoPara|' in chk and "'📎 comprovante da '" in chk and '_gradeMeta={titulo:' in chk,
 # [revisao F3 23/08 — G1 prazo de 8 s + pendNuvem na recusa; G2 o fundo do visor nao fecha com o painel aberto; M1 nao duplica (le o destino antes); M2/M3 contador por releitura, nunca ±1 cego; M4 "incluir antigos"; M5 confirm antes de mover]
 'fotos F3b: prazo de socorro, recusa com pendNuvem, fundo nao fecha, sem duplicar, contador relido, incluir antigos, confirm':'A nuvem não respondeu em 8 segundos' in chk and '||pendNuvemVelha())' in chk and 'function pendNuvemVelha()' in chk and 'v.onclick=null;}' in chk and 'D&&D.some(x=>x.b64===f.b64)' in chk and 'function recontarFotos(movId)' in chk and chk.count('recontarFotos(')>=2 and 'const armar=()=>{' in chk and 'fotoList(destino,D=>{if(respondeu)return;armar();' in chk and 'incluir lançamentos com mais de 90 dias' in chk and "if(!confirm('Mover esta foto para:" in chk and "const lim=tudo?'':" in chk and 'Nenhum lançamento ${tudo?' in chk and 'if(+mOri.nFotos>0){mOri.nFotos=+mOri.nFotos-1;save();}' in chk,
 # [fotos F5a 25/08 — decisao do Felype: foto de excluido morre aos 120 dias; fila de reenvio persistida (IDB v2); medidor de espaco no Backup; guardas de pre-login (defeito real da F2 achado nesta fatia)]
 'fotos F5a: IDB sobe de versao com os stores fila e rascunho (upgrade so cria o que falta)':"indexedDB.open('tcg_fotos',3)" in chk and "createObjectStore('fila',{keyPath:'qid'})" in chk and "createObjectStore('rascunho',{keyPath:'id'})" in chk and 'rq.onblocked=()=>cb(null);' in chk,
 'fotos F5a: fila de foto recusada persistida (grava ao entrar, solta ao gravar, volta na abertura)':'function filaFotoGrava(f)' in chk and 'function filaFotoSolta(f)' in chk and 'function carregarFilaFotos()' in chk and 'filaFotoGrava(fq);' in chk and 'filaFotoSolta(f);contarFotoGravada' in chk and 'carregarFilaFotos();}catch(e){}' in chk,
 'fotos F5a: purga de fotos de excluidos aos 120 dias (fila persistida, varredura de fundo, poucos por vez)':'let _fotosPurga=[]' in chk and "gravaLocal('tcg_fotos_purga'" in chk and '_fotosPurga.push(dono)' in chk and 'function processarPurgaFotos()' in chk and '_fotosPurga.slice(0,3)' in chk and 'processarPurgaFotos();}catch(e){}' in chk and 'retomarMovesPendentes();processarPurgaFotos();})' in chk,
 'fotos F5a: com nuvem ligada e sem login, reenvio e retomada ESPERAM (senao gravavam no armazenamento errado)':chk.count('if(USAR_NUVEM&&!_db)return')>=3,
 'fotos F5a: medidor de espaco no Backup (documento em bytes + fotos por fora) e texto honesto dos 120 dias':'function medirDocBytes()' in chk and 'function medirFotosNuvem(el)' in chk and 'Espaço na nuvem — documento principal' in chk and 'medir o espaço das fotos' in chk and 'por 120 dias' in chk and "_fotosFalhadas.some(f=>f.enviando||!f.salva)" in chk,
 # [revisao F5a 25/08 — ajustes do revisor: purga pula dono vivo; frase da exclusao sem promessa falsa; medidor mede o payload real e diz a verdade sobre o teto; todasFotos null em erro (backup completo aborta, medidor avisa); fila com confirmacao de cofre; porta de saida no login]
 'fotos F5b: purga pula dono que voltou a existir (restauro/import/fusao/id reusado) sem apagar nada':'movs.some(x=>x.id===dono||x.notaId===dono||x.vendaId===dono||x.trocaId===dono)' in chk,
 'fotos F5b: exclusao diz a verdade (120 dias + Lixeira restauravel), sem a promessa falsa antiga':'vai para a 🗑 Lixeira' in chk and 'o prazo em que dá pra restaurar' not in chk,
 # [lixeira 25/08 — decisao do Felype: "tenho que conseguir acessar um campo de lixeira/restauracao, senao e so uma funcao que existe"]
 'lixeira: IDB v3 com stores lixeira e diario; nuvem em subcolecoes proprias (fora do documento de 1 MB)':"createObjectStore('lixeira',{keyPath:'id'})" in chk and "createObjectStore('diario',{keyPath:'id'})" in chk and "collection('lixeira')" in chk and "collection('diario')" in chk,
 'lixeira: toda porta de exclusao guarda o lancamento inteiro (so este, nota inteira, devolucao, apagar tudo)':'lixeiraGuarda(removidos);' in chk and 'lixeiraGuarda(its);' in chk and chk.count('lixeiraGuarda([v]);')>=2 and 'lixeiraGuarda(movs);' in chk,
 'lixeira: tela com restaurar (tumulos+fila de purga+entrada), aviso de venda vinculada, leitura falhada AVISA':'function abrirLixeira()' in chk and 'function lixeiraRestaura(id)' in chk and 'NÃO baixa o estoque de novo' in chk and 'Não consegui ler a lixeira agora' in chk and 'some em ' in chk and '|lixeiraRestaura|lixeiraApaga|' in chk and 'fecharModal();abrirLixeira()' in chk,
 'lixeira: a limpeza dos 120 dias apaga a entrada junto com as fotos; diario registra quem excluiu/devolveu/restaurou':chk.count('lixeiraApaga(dono,function(){})')>=2 and chk.count('🗑 Lixeira')>=4 and 'function diarioReg(' in chk and "diarioReg('excluiu'" in chk and "diarioReg('devolveu venda'" in chk and "diarioReg('restaurou da lixeira'" in chk and "diarioReg('apagou tudo'" in chk,
 # [G1-classe 25/08 — restauracao viaja com SINAL; fusao do dicionario de exclusao por evento mais recente (revisor-lixeira G1, rodada propria)]
 'G1-classe: desmarcar grava marca negativa, estaExcluido exige positivo, fusao por |ts| maior NAS DUAS PORTAS, carimbo monotonico, aviso de aba velha, poda por |idade|':'function mergeExcl(a,b)' in chk and 'excluidos[id]=-t;' in chk and 'function estaExcluido(id){return !!(id&&+excluidos[id]>0);}' in chk and 'mergeExcl(excluidos,r.excluidos)' in chk and 'excluidos=mergeExcl(excluidos,d.excluidos);' in chk and 'function agoraExcl()' in chk and chk.count('const t=agoraExcl();')>=2 and "_v:(typeof BUILD_TS!=='undefined'?BUILD_TS:0)" in chk and 'Há uma versão mais nova do app' in chk and 'Math.abs(v)>lim' in chk and 'if(v>0){const dono=' in chk and 'const lim=ref-120*864e5;' in chk,
 # [1MB fase 1 25/08 — miniatura fora do documento (item 3 da fila: reengenharia do limite de 1MB)]
 '1MB fase 1: payload/medidor enxutos, hash cego a miniatura, reidratacao nas 2 portas, docs th_ na subcolecao fotos, pendencia que drena, backfill 1x, backup filtra th_':'function semThumbs(arr)' in chk and 'function reidrataThumbs(antes,depois)' in chk and 'movs:semThumbs(final.movs)' in chk and 'movs:semThumbs(movs)' in chk and 'if(o&&o.fotoThumb){const c={...o};delete c.fotoThumb;o=c;}' in chk and 'movs:reidrataThumbs(movs,M(movs,r.movs))' in chk and 'reidrataThumbs(_movsAntesTh,movs);' in chk and "ref.doc('th_'+id).set({t:1,th,ts:Date.now()})" in chk and "ref.where('t','==',1).get()" in chk and 'function backfillThumbs()' in chk and "localStorage.getItem('tcg_th_bf1')" in chk and 'if(x&&x.b64)L.push(Object.assign({id:d.id},x));' in chk and chk.count('thumbNuvem(')>=10 and 'const TH_MARCA=' in chk and 'c.fotoThumb=TH_MARCA;' in chk and 'if(q[id]===th)delete q[id];' in chk and 'if(n&&!thumbPendGrava(p))return;' in chk and "return gravaLocal('tcg_th_pend'" in chk and 'function thumbReal(x)' in chk and 'const th=thumbReal(m.fotoThumb)||' in chk and chk.count('thumbReal(')>=10,
 # [F2c 25/08 — a foto tirada pro leitor de codigo pode virar a foto DESTA carta (1 toque)]
 'F2c: foto do scanner reaproveitada (balde CERTO por forma: f_cod->_fotosPend/r_cod->_fotosItem, link amarrado ao hint, reanexado em ligaHint/assistCod/aplicarScan, todas as saidas do leitor)':'function usarFotoScan(hintId)' in chk and 'function scanFotoLnk(hintId)' in chk and 'function reanexaScanLnk(hintId)' in chk and "if(codId==='f_cod'||codId==='r_cod')" in chk and "cvf.toDataURL('image/jpeg',0.72)" in chk and "form:codId,hintId:hintId}" in chk and "s.hintId!==hintId)return '';" in chk and "if(s.form==='f_cod'){_fotosPend.push(s.b64);}else{_fotosItem.push(s.b64);}" in chk and chk.count('reanexaScanLnk(')>=4 and "if(!w){if(hint)hint.innerHTML=scanFotoLnk(hintId);return;}" in chk and 'Foto guardada no lançamento' in chk and 'function ligaHint(cod,elId){_ligaHint(cod,elId);reanexaScanLnk(elId);}' in chk and 'if(_fotosPend.length)aplicarFotosPend(movs[i].id);' in chk,
 # [F4 25/08 — rascunho persistente do Lancar (cura da queixa de 21/08 "fechei a aba e perdi")]
 'F4: rascunho persiste no aparelho (debounce no render, gate anti-sobrescrita do boot), restaura sem mexer na tela, limpa nos salvamentos':'function dadosRascunho()' in chk and 'function agendaRascunho(){if(!_rascPronto)return;' in chk and 'setTimeout(gravaRascunho,400)' in chk and 'if(_telaLoginNoAr)return;agendaRascunho();' in chk and 'function restauraRascunho()' in chk and 'Rascunho recuperado' in chk and chk.count('limpaRascunho();')>=5 and "setTimeout(()=>{try{restauraRascunho();}catch(e){_rascPronto=true;}},800);" in chk and "createObjectStore('rascunho',{keyPath:'id'})" in chk,
 # [diario 25/08 — a tela do "quem fez o que" + gestos de criar registrando]
 'diario: gestos de criar/pagar/desfazer/separar/juntar/restaurar/mover registram quem fez':"diarioReg('lançou',rotDe(obj)" in chk and "diarioReg('editou',rotDe(movs[i])" in chk and "diarioReg('lançou nota de compra'" in chk and "diarioReg('vendeu (vários)'" in chk and "diarioReg('registrou troca'" in chk and "diarioReg('editou troca'" in chk and "diarioReg('marcou pago'" in chk and "diarioReg('desfez nota'" in chk and "diarioReg('separou da nota'" in chk and "diarioReg('juntou em nota'" in chk and "diarioReg('juntou notas'" in chk and "diarioReg('restaurou backup local'" in chk and "diarioReg('restaurou ponto da nuvem'" in chk and "diarioReg('moveu foto'" in chk and "diarioReg('pagou parcela'" in chk and "diarioReg('corrigiu a nota'" in chk and "diarioReg('registrou transferência'" in chk and "diarioReg('desvinculou venda'" in chk and "diarioReg('excluiu (lote)'" in chk and "diarioReg('marcou pedido chegado'" in chk and "diarioReg('excluiu pessoa do catálogo'" in chk,
 'diario: tela com filtro por pessoa, erro avisa (nunca finge vazio), porta no Backup':'function abrirDiario()' in chk and 'function diarioLista(cb)' in chk and 'function desenhaDiario()' in chk and 'Não consegui ler o diário agora' in chk and 'fecharModal();abrirDiario()' in chk and 'filtraDiario(' in chk,
 # [revisao lixeira 25/08 — G1 restaurar mantem a entrada (fusao entre aparelhos pode devorar o restaurado); G3 prova real apita venda vinculada a item que voltou pro estoque]
 'lixeira G1/G3: entrada fica apos restaurar; prova real apita o dobro da venda restaurada pos-devolucao':"save();diarioReg('restaurou da lixeira'" in chk and "lixeiraApaga(id,function(){});diarioReg" not in chk and 'conta DUAS vezes' in chk and "sitDe(alvo)!=='Vendido'&&sitDe(alvo)!=='Trocado'" in chk,
 'fotos F5b: medidor mede o payload REAL (com excluidos) e nao inventa trava que nao existe':'contasBanc,codigosResolvidos,excluidos});try{return new TextEncoder' in chk and 'NÃO se trava perto do teto' in chk and 'teto técnico de 1 MB' in chk and 'o app trava o salvamento em 700 mil bytes' not in chk,
 'fotos F5b: leitura de todas as fotos falhada = null; backup completo ABORTA avisando; medidor avisa':chk.count('.catch(()=>cb(null))')>=2 and 'O backup completo sairia SEM elas' in chk and 'não consegui ler as fotos agora' in chk,
 'fotos F5b: fila confirma o cofre (f.salva) e o login e porta de saida do reenvio/retomada/purga':'tx.oncomplete=()=>{f.salva=true;}' in chk and 'f.salva=true;_fotosFalhadas.push(f)' in chk and 'try{setTimeout(()=>{try{reenviarFotosFalhadas();}catch(e){}try{retomarMovesPendentes();}catch(e){}try{processarPurgaFotos();}catch(e){}' in chk and 'try{backfillThumbs();}catch(e){}try{despachaThumbs();}catch(e){}try{carregarThumbsNuvem();}catch(e){}},2500);}catch(e){}' in chk,
 # [fotos F1 23/08 — P0-1..P0-5: ler fotos sem rede nao zera; foto so conta depois de gravada; visor nao grava a toa; venda 1<->varios no balde; portao de testes fail-closed]
 'fotos F1: leitura falhada = null (nao zera contador), balde so esvazia com confirmacao, vendaModo na identidade':'.catch(()=>cb(null));}' in chk and 'if(L===null){_fotosCache=[];' in chk and ('let _fotosEmVoo=[],_fotosFalhadas=[];' in chk or 'let _fotosEmVoo=[],_fotosFalhadas=[],_gradeDe=null;' in chk) and ('_fotosFalhadas.push({movId,b64' in chk or '_fotosFalhadas.push(fq);filaFotoGrava(fq);' in chk) and 'function reenviarFotosFalhadas' in chk and 'if(_fotosFalhadas.length)reenviarFotosFalhadas();' in chk and 'f.enviando=true;' in chk and 'function fotoAdd(movId,b64,cb,silencioso)' in chk and "addEventListener('beforeunload'" in chk and "(typeof vendaModo==='undefined'?'':vendaModo)+'|'" in chk and 'q.metadata.fromCache&&q.empty){cb(null);return;}' in chk,
 'fotos F1: portao de testes fail-closed nas 3 portas (publicar, pre-commit, CI) com minimo declarado':all('testes-nucleo.minimo' in open(os.path.join(AQUI,p),encoding='utf-8').read() for p in ('publicar.sh','.githooks/pre-commit','.github/workflows/app-sintaxe.yml')) and os.path.exists(os.path.join(AQUI,'testes-nucleo.minimo')),
 ...
 'fusao com o remoto: tumulos ANTES de unir, uma funcao pras duas portas (salvarNuvem e aplicarNuvem)':'final=fundirComRemoto(remoto)' in chk and ('mergeDictRaso(excluidos,r.excluidos)' in chk or 'mergeExcl(excluidos,r.excluidos)' in chk) and 'function gravaTudoLocal' in chk and "gravaLocal('tcg_excluidos',JSON.stringify(excluidos));}" in chk,
 ...
 'login: render nao pinta por cima da tela de login (app ficava intransponivel deslogado)':'let _telaLoginNoAr=false' in chk and ("function render(){guardaBaldeItem();if(_telaLoginNoAr)return;" in chk or "function render(){guardaBaldeItem();guardaBaldePend();if(_telaLoginNoAr)return;" in chk) and 'function telaLogin(msg){_telaLoginNoAr=true;' in chk and '_syncReady=true;_telaLoginNoAr=false;' in chk,
