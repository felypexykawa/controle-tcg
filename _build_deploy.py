...
checks={
 ...
 'conta, colecao e jogo apagados tambem nao ressuscitam':"marcaExcluido(id);contasBanc=contasBanc.filter" in chk and "mergeArrUniao(cols,remoto.cols,'cols:')" in chk and "mergeArrUniao(jogos,remoto.jogos,'jogos:')" in chk and "mergeArrUniao(pgs,remoto.pgs,'pgs:')" in chk,
 ...
 'registro de exclusao viaja pela nuvem (payload + merge + volta)':'codigosResolvidos,excluidos};' in chk and 'excluidos:mergeDictRaso(excluidos,remoto.excluidos),' in chk and "if(d.excluidos&&typeof d.excluidos==='object')" in chk and "gravaLocal('tcg_excluidos'" in chk,
 'catalogo apagado tambem nao ressuscita (fornecedor/cliente)':'function mergeArrUniao(local,remoto,pref){' in chk and 'if(pref&&estaExcluido(pref+v))return;' in chk and "mergeArrUniao(pess,remoto.pess,'pess:')" in chk and "marcaExcluido('pess:'+nome," in chk and "marcaExcluido('cols:'+c)" in chk and "marcaExcluido('jogos:'+j)" in chk,
 ...
 'filtro sticky em todo lugar + rola so ao abrir (nao ao fechar)':chk.count('position:sticky;top:var(--hh,0px);z-index:4')>=4 and "aberto?'':\";window.scrollTo({top:0,behavior:'smooth'})\"" in chk and chk.count("window.scrollTo({top:0,behavior:'smooth'})") >= 5,
